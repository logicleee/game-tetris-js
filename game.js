...
function Board(size = [10,20]) {
  ...
  this.renderGridAsText = (grid = this.grid, size = this.size) => {
    const maxX = size[0];
    let j = 0;
    let result = '';
...
function Controller () {
  ...
  */
  ...
  function run () {
    ...
    function frame () {
      ...
      last = now;
        sleep(50).then(() => {
          if (playingGame)
                setElementInnerText('score', timeStamp());
                //setElementInnerText('score', timeDelta);
  ...
  function draw () {
    if (boardChanged) {
      setElementInnerText('canvasDiv', board.renderGridAsText(textGrid));
      boardChanged = false;
    }
